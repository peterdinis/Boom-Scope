	...
	loading: () => (
		<div className="flex h-full w-full items-center justify-center bg-background">
			<div className="flex flex-col items-center gap-4">
				...
				<p className="text-[10px] font-bold text-foreground/40 tracking-[0.3em] uppercase animate-pulse">Syncing Engine</p>
			</div>
		</div>
	)
});
				...
				{leftPanelOpen && (
					...
					>
						<div className="flex items-center justify-between mb-10">
							...
							<Button variant="ghost" size="icon-xs" onClick={() => setLeftPanelOpen(false)} className="hover:bg-accent rounded-lg">
								...
								elements.map((el, i) => (
									<div key={el.id} className="relative group">
										...
										>
											<div className="flex items-center gap-4">
												<div className={cn(
													...
													selectedId === el.id ? "bg-white/20" : "bg-background/40"
												)}>
										...
										<div className={cn(
											...
											>
												{el.isVisible === false ? <EyeOff className="size-3 text-red-500" /> : <Eye className="size-3 opacity-40" />}
											</button>
											<button 
												onClick={(e) => { e.stopPropagation(); toggleElementProperty(el.id, 'isLocked'); }}
												className="p-1.5 rounded-lg hover:bg-accent transition-colors"
											>
												{el.isLocked ? <Lock className="size-3 text-amber-500" /> : <Unlock className="size-3 opacity-40" />}
											</button>
										</div>
									</div>
								)).reverse()
				...
				{rightPanelOpen && (
					...
					>
						<div className="flex items-center justify-between mb-10">
							...
							<Button variant="ghost" size="icon-xs" onClick={() => setRightPanelOpen(false)} className="hover:bg-accent rounded-lg">
									...
									<div className="grid grid-cols-2 gap-3 p-1 rounded-2xl bg-accent border border-border shadow-inner">
										<button 
											...
											className={cn(
												...
												selectedElement.isVisible === false ? "bg-red-500/20 text-red-500" : "hover:bg-background/50 text-foreground/50"
											)}
										>
										...
										<button 
											...
											className={cn(
												"flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all",
												selectedElement.isLocked ? "bg-amber-500/20 text-amber-500" : "hover:bg-background/50 text-foreground/50"
											)}
										>
											...
											<div className="space-y-8 animate-in fade-in duration-500">
												...
												<div className="space-y-5">
													<div className="flex justify-between items-center">
														...
														<span className="text-[10px] font-mono font-bold opacity-60">{selectedElement.fontSize}px</span>
													</div>
													<select 
													...
													>
														{FONTS.map(f => (
															<option key={f} value={f} className="bg-background text-foreground">{f.split(',')[0]}</option>
														))}
													</select>
													<input 
														type="range" 
						...
						<div className="mt-10 pt-8 border-t border-border grid grid-cols-2 gap-4">
							<Button variant="outline" className="h-14 rounded-[20px] bg-background border-border hover:bg-accent transition-all shadow-sm" onClick={() => handleAction("undo")}>
								<Undo2 className="size-5 opacity-40" />
							</Button>
							<Button variant="outline" className="h-14 rounded-[20px] bg-background border-border hover:bg-accent transition-all group shadow-sm" onClick={() => handleAction("trash")}>
