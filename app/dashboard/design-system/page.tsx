...
import { 
	Upload, Sparkles, Palette, Type, Layout, Copy, Check, RefreshCw, 
	Image as ImageIcon, Trash2, ArrowRight, Download, Wand2 
} from "lucide-react";
...
export default function DesignSystemPage() {
	...
	const extractColors = async (imageUrl: string): Promise<string[]> => {
		return new Promise((resolve) => {
			...
			img.onload = () => {
				...
				if (!ctx) return resolve(["#3b82f6", "#10b981", "#6366f1", "#f59e0b", "#ef4444"]);
				...
				resolve(sortedColors.length >= 5 ? sortedColors : [...sortedColors, "#3b82f6", "#10b981"].slice(0, 5));
			};
		});
	};
	...
	const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		files.forEach(file => {
			...
			reader.onload = () => {
				setImages(prev => [...prev, { id: Math.random().toString(), url: reader.result as string }]);
			};
	...
	const analyzeImages = async () => {
		...
		const randomFontPair = FONT_PAIRS[Math.floor(Math.random() * FONT_PAIRS.length)];

		setSystem({
			colors: uniqueColors.length >= 5 ? uniqueColors : ["#3b82f6", "#10b981", "#6366f1", "#f59e0b", "#ef4444"],
			fonts: randomFontPair,
			vibe: ["Modern Minimalist", "Bold & Vibrant", "Elegant Luxury", "Tech Brutalist"][Math.floor(Math.random() * 4)]
		});
							...
							>
								{isAnalyzing ? (
									...
									<div className="flex items-center gap-3">
										...
										<span className="font-black uppercase tracking-widest text-xs">Generovať Identitu</span>
									</div>
								)}
							</Button>
						</motion.div>
						...
						{images.length === 0 ? (
							<div className="h-full flex flex-col items-center justify-center space-y-6">
								...
								<div className="text-center">
									<p className="text-sm font-black uppercase tracking-[0.2em] opacity-40">Presuňte inšpiráciu sem</p>
									<p className="text-[10px] font-bold opacity-20 uppercase tracking-widest mt-2">Podporuje PNG, JPG, WebP</p>
								</div>
							</div>
						) : (
							<div className="h-full flex items-center gap-4 px-8 overflow-x-auto custom-scrollbar">
								{images.map((img) => (
									<div key={img.id} className="relative h-48 min-w-48 rounded-[28px] overflow-hidden group/img shadow-2xl border border-white/10">
										<img src={img.url} className="h-full w-full object-cover transition-transform duration-700 group-hover/img:scale-110" alt="Inspiration" />
										<button 
					...
					{isAnalyzing && (
						...
						>
							...
							<div className="text-center space-y-4">
								<h2 className="text-3xl font-black tracking-tight uppercase">Analyzujem vizuálnu DNA</h2>
								<p className="text-foreground/40 font-medium tracking-widest text-[10px] uppercase animate-pulse">Extrahujem farby • Mapujem typografiu • Generujem komponenty</p>
							</div>
						</motion.div>
					)}
						...
						>
							...
							<div className="lg:col-span-4 space-y-8">
								<div className="p-8 rounded-[40px] bg-background/40 backdrop-blur-3xl border border-border shadow-2xl space-y-8">
									<div className="space-y-2">
										<p className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-500">Vizuálny Štýl</p>
										<h2 className="text-4xl font-black tracking-tight">{system.vibe}</h2>
									</div>
									...
									<div className="space-y-6">
										<div className="flex items-center gap-4">
											...
											<span className="text-[10px] font-black uppercase tracking-widest opacity-40">Farebná Paleta</span>
										</div>
										<div className="space-y-3">
											{system.colors.map((color) => (
												...
												>
													<div className="flex items-center gap-4">
														<div className="size-8 rounded-lg shadow-inner" style={{ backgroundColor: color }} />
														<span className="font-mono text-xs font-bold uppercase tracking-wider opacity-60">{color}</span>
													</div>
													{copiedColor === color ? <Check className="size-4 text-emerald-500" /> : <Copy className="size-4 opacity-0 group-hover:opacity-20 transition-opacity" />}
												</button>
											))}
										</div>
									</div>
									...
									<div className="pt-8 border-t border-border/50 space-y-6">
										<div className="flex items-center gap-4">
											...
											<span className="text-[10px] font-black uppercase tracking-widest opacity-40">Typografia</span>
										</div>
										<div className="space-y-4">
											<div className="p-5 rounded-2xl bg-foreground/5 border border-border/50">
												<p className="text-[8px] font-black uppercase tracking-[0.2em] opacity-20 mb-2">Nadpisy</p>
												<p className="text-xl font-black" style={{ fontFamily: system.fonts.heading }}>{system.fonts.heading}</p>
											</div>
											<div className="p-5 rounded-2xl bg-foreground/5 border border-border/50">
												<p className="text-[8px] font-black uppercase tracking-[0.2em] opacity-20 mb-2">Text</p>
												<p className="text-base font-medium" style={{ fontFamily: system.fonts.body }}>{system.fonts.body}</p>
											</div>
										</div>
									</div>
							...
							<div className="lg:col-span-8 space-y-8">
								<div className="p-10 lg:p-16 rounded-[40px] bg-background/40 backdrop-blur-3xl border border-border shadow-2xl space-y-16 overflow-hidden relative">
									...
									<div className="space-y-4">
										<p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500">Live Preview</p>
										<h2 className="text-4xl font-black tracking-tight">Komponenty</h2>
									</div>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-12">
										...
										<div className="space-y-8">
											<p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-20">Tlačidlá</p>
											<div className="space-y-4">
												<Button 
												...
												<Button 
													...
													style={{ borderColor: system.colors[1], color: system.colors[1] }}
												>
													Secondary Option
												</Button>
											</div>
										...
										{/* Inputs */}
										<div className="space-y-8">
											<p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-20">Inputy</p>
											<div className="space-y-4">
												<div className="relative">
													<div className="absolute left-4 top-1/2 -translate-y-1/2 opacity-20" style={{ color: system.colors[0] }}>
												...
												<div className="flex gap-2">
													{system.colors.map(c => (
														<div key={`swatch-${c}`} className="size-6 rounded-lg shadow-sm" style={{ backgroundColor: c }} />
													))}
												</div>
											</div>
										</div>
										...
										<div className="md:col-span-2 space-y-8">
											<p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-20">Karty & Layout</p>
											<div 
											...
											>
												<div className="flex items-center gap-4">
													<div className="size-12 rounded-2xl flex items-center justify-center text-white" style={{ backgroundColor: system.colors[2] }}>
														<Layout className="size-6" />
													</div>
													<div>
														<h4 className="font-black text-lg" style={{ fontFamily: system.fonts.heading }}>Vizuálna Integrita</h4>
														<p className="text-xs font-medium opacity-40" style={{ fontFamily: system.fonts.body }}>Harmonické farby extrahované z vašej inšpirácie.</p>
													</div>
												</div>
												<div className="flex gap-4">
													<div className="flex-1 h-2 rounded-full opacity-10" style={{ backgroundColor: system.colors[0] }} />
													<div className="w-1/3 h-2 rounded-full opacity-10" style={{ backgroundColor: system.colors[3] }} />
												</div>
											</div>
										</div>
									</div>
